...
test("shared Merch routes and alert forms stay contractually connected",()=>{
  ...
  assert.match(alertsCss,/grid-template-columns:1fr!important/);assert.match(alertsCss,/--src-color-bright/);assert.match(alertsCss,/input\[type="email"\].*outline:0/);assert.match(merchApi,/AbortSignal\.timeout\(4000\)/);
