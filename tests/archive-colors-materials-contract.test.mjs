...
test("visual source discovery excludes Tattoo context images and merges only allowed atomic suggestions",async()=>{
  ...
  const calls=[];
  const assetFetches=[];
  const analysisAssets={fetch:async request=>{assetFetches.push(request.url);return new Response(Uint8Array.from([0xff,0xd8,0xff,0xd9]),{headers:{"content-type":"image/jpeg"}})}};
