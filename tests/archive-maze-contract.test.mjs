...
test("Maze Archive migration seeds only public collection shells and a private authored-history draft",()=>{
  ...
  const pattern=database.prepare("SELECT ce.visibility,ce.search_visibility,ar.state record_state,ad.state dossier_state,ad.public_visible,ot.state origin_state,ot.public_visible origin_visible FROM content_entities ce JOIN archive_records ar ON ar.id=ce.id JOIN archive_dossiers ad ON ad.entity_id=ce.id JOIN archive_origin_thread_entities ote ON ote.entity_id=ce.id JOIN archive_origin_threads ot ON ot.id=ote.thread_id WHERE ce.id='archive-maze-pattern'").get();
