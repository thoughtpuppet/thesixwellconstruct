...
test("Lost Marbles Note is one public identity across Notes, Notebook, Origin Thread, and search",async()=>{
  ...
  const detail=await json(await handleConstructApi(request("/api/archive/notes/lost-marbles-inception-note"),runtime));assert.equal(detail.status,200);assert.equal(detail.body.note.source_created_at,"2022-12-23T19:54:00-05:00");assert.equal(detail.body.assets.length,2);assert.deepEqual(detail.body.assets.map(asset=>asset.token),["original-sketch","process-experiment"]);assert.equal(JSON.stringify(detail.body).includes("note-source-pdf"),false);assert.match(detail.body.assets[1].caption,/possible reference for another work/);
  ...
  const searchDocument=db.prepare("SELECT * FROM search_documents WHERE entity_id=?").get(noteId);assert.equal(searchDocument.entity_type,"archive_note");assert.match(searchDocument.body,/possible reference for another work/);
...
test("Archive Notes UI keeps the Apple date reminder, bounded reader, and clean export boundary",()=>{
  const studio=readFileSync(join(ROOT,"studio","archive-notes-manager.js"),"utf8"),styles=readFileSync(join(ROOT,"css","archive-notes.css"),"utf8"),page=readFileSync(join(ROOT,"archive","notes","index.html"),"utf8"),migration=readFileSync(join(ROOT,"migrations","0177_archive_notes.sql"),"utf8");
  ...
  assert.match(styles,/max-height:min\(72dvh,900px\)/);assert.match(styles,/overflow-y:auto/);assert.match(styles,/border:5px/);assert.match(page,/data-archive-view="notes"/);assert.match(page,/archive-note-markdown\.js/);assert.match(migration,/archive_note_assets/);assert.match(migration,/archive_note_links/);
});
