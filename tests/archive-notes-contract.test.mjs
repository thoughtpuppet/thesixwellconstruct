...
test("Archive Notes UI keeps the Apple date reminder, bounded reader, and clean export boundary",()=>{
  ...
  assert.match(styles,/max-height:min\(72dvh,900px\)/);assert.match(styles,/overflow-y:auto/);assert.match(styles,/border:5px/);assert.match(styles,/archive-note-provenance/);assert.match(styles,/color:var\(--color-text-dim\)!important/);assert.match(styles,/text-transform:none!important/);assert.match(styles,/archive-note-image-dialog/);assert.match(renderer,/data-note-image-trigger/);assert.match(renderer,/bindImageLightboxes/);assert.match(notesPage,/bindImageLightboxes\(app\)/);assert.match(archivePublic,/bindImageLightboxes\(content\)/);assert.match(page,/data-archive-view="notes"/);assert.match(page,/archive-note-markdown\.js/);assert.match(migration,/archive_note_assets/);assert.match(migration,/archive_note_links/);
});
