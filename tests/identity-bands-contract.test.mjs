...
test("identity-band landings share one stylesheet and explicit structure", async () => {
  ...
  for (const html of pages) {
    ...
    assert.match(html, /<div class="band-identity">\s*<a class="band-identity-link" href="\/about\/legend\/\?symbol=[^"]+" aria-label="[^"]+">\s*<img class="band-mark"[^>]*>\s*<\/a>\s*<\/div>\s*<div class="band-content">\s*<p class="band-kicker"[\s\S]*?<h2 class="band-title"/);
  ...
  assert.match(
    pages[2],
    /<section class="artist-band[^"]*"[\s\S]*?<div class="band-identity band-portrait">\s*<img[^>]*>\s*<\/div>\s*<div class="band-content">\s*<p class="band-kicker"[\s\S]*?<h2 class="band-title"[\s\S]*?<p class="band-copy"/,
  );
});
...
test("shared identity actions are filled, accessible, responsive controls", async () => {
  ...
  assert.match(
    identityCss,
    /\.brand-band\s*\{[\s\S]*--band-identity-column:\s*minmax\(160px,\s*0\.25fr\)[\s\S]*--band-content-column:\s*minmax\(0,\s*0\.75fr\)[\s\S]*--band-column-gap:\s*0[\s\S]*--band-inline-padding:\s*8px/,
  );
  assert.match(
    identityCss,
  ...
  assert.match(identityCss, /\.brand-band \.band-identity\s*\{[\s\S]*aspect-ratio:\s*4\s*\/\s*5/);
