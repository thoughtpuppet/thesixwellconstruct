...
test("Gallery surfaces preserve the shared shell and expose the complete relational workflow", () => {
  ...
  const adminCollapse = readFileSync(join(ROOT, "studio", "admin-collapse.js"), "utf8");
  const resumableClient = readFileSync(join(ROOT, "studio", "resumable-media-upload.js"), "utf8");
  ...
  assert.match(studio, /Use an MP4 or WebM video/);
  assert.match(studio, /shouldUseArchiveResumable/);
  assert.match(studio, /uploadKind:"archive-master",archiveScope:"archive"/);
  assert.match(studio, /directUploadMaxBytes=15\*1024\*1024/);
  assert.match(studio, /managedUploadAccept="[^"]*video\/mp4,video\/webm/);
  assert.doesNotMatch(studio, /managedUploadAccept="[^"]*video\/\*/);
  assert.match(resumableClient, /archiveScope:options\.archiveScope\|\|""/);
  assert.match(studio, /data-detail-workspace/);
  assert.match(studio, /data-admin-collapse-ignore/);
  assert.match(adminCollapse, /closest\("\[data-admin-collapse-ignore\]"\)/);
  assert.doesNotMatch(studio, /<label>Role<select name="asset_role"/);
  assert.doesNotMatch(studio, /<label>Media role<input name="role"/);
  assert.match(studio, /body\.asset_role=automaticAssetRole/);
  assert.match(studio, /body\.role="documentation"/);
  ...
  assert.match(studioShell, /construct-manager\.js\?v=20260904-media-detail-cleanup/);
  assert.match(studioShell, /media-catalogue-manager\.css\?v=20260904-media-detail-cleanup/);
  assert.match(studioShell, /admin-collapse\.js\?v=20260904-media-detail-cleanup/);
  assert.match(studioShell, /resumable-media-upload\.js\?v=20260904-large-media/);
  assert.match(studioManager, /media-catalogue-manager\.js\?v=20260904-media-detail-cleanup/);
  ...
  assert.match(studioStyles,/\.mcm-preview-large img\{[^}]*width:auto;max-width:100%;height:auto;max-height:none;object-fit:contain/);
  assert.match(studioStyles,/\.mcm-preview-large\[data-preview-state="unavailable"\]::after/);
