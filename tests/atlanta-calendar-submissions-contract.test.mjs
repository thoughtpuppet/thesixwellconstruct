...
function database(){const db=new DatabaseSync(":memory:");db.exec("PRAGMA foreign_keys=ON");for(const name of readdirSync(join(ROOT,"migrations")).filter((name)=>name.endsWith(".sql")&&name!=="0147_calendar_creative_scout_import.sql").sort())db.exec(readFileSync(join(ROOT,"migrations",name),"utf8"));return db;}
