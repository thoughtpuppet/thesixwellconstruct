(function () {
  ...
  var PUBLIC_TEMPLATES = [
    ...
    { id: "merch-product", label: "Merch product detail", kind: "commerce-detail", accent: "merch", route: "/merch/lostmarbles-hoodie/", sources: ["merch/detail/index.html", "css/merch-detail.css"], description: "Shared Studio-backed product detail using the original LOST MARBLES. Hoodie anatomy, with availability, launch alerts, commerce controls, and origin records." },
    ...
    { id: "archive-place-dossier", label: "Archive place dossier", kind: "editorial-dossier", accent: "archive", route: "/archive/places/jr-erikson-building/", routes: ["/archive/places/", "/archive/places/jr-erikson-building/", "/archive/places/goat-farm-arts-center/", "/archive/places/guardian-studios/"], sources: ["archive/places/index.html", "archive/places/jr-erikson-building/index.html", "archive/places/goat-farm-arts-center/index.html", "archive/places/guardian-studios/index.html", "css/archive-place-study.css"], description: "Source-led place lens with stable public routes, verified and inferred claim labels, documentary evidence, chronology, open questions, and linked source registers; places remain contextual rather than cultural-object records." },
    ...
    { id: "construct-explore", label: "Explore / random discovery", kind: "interactive", accent: "about", route: "/adventure/", heroVariant: "Immersive room", sources: ["adventure/index.html", "css/explore.css", "js/explore.js", "js/explore-room.js", "js/construct-ambient-field.js", "functions/api/construct/_lib.js", "js/construct-nav.js"], adoption: "Immersive public interactive template adopted · separate from the nine medium nodes", description: "Balanced cross-Construct discovery staged as an empty room with semantic sculptural controls, a ghosted eye field, restrained particles, immediate navigation, history exclusions, and retryable feedback." },
  ...
  PUBLIC_TEMPLATES.forEach(function (record) {
    if (!record.heroVariant) {
      record.heroVariant = record.family === "Landing Family" ? "Medium landing hero" : "Supporting hero";
    }
    if (record.heroVariant !== "Immersive room" && record.sources.indexOf("css/hero.css") === -1) {
      record.sources.push("css/hero.css");
    }
  });
  ...
  var COMPONENTS = [
    ...
    { id: "component-commerce", label: "Commerce", system: "public", kind: "component-commerce", sources: ["merch/index.html", "merch/detail/index.html"], description: "Product card, shared detail shell, launch alerts, variant, price, inventory, cart line, subtotal, and checkout feedback." },
  ...
  var STUDIO_PATTERNS = [
    ...
    { id: "studio-media", label: "Portfolio / media manager", kind: "studio-media", route: "/studio/submissions/#tattoo", sources: ["studio/submissions/index.html", "studio/construct-manager.js"], description: "Drop zone, cards, image roles, privacy and inclusion controls, editors, sort actions, and upload feedback." },
  ...
  function canonicalCalendarSection() {
    return '<div class="booking-layout canonical-booking-calendar" aria-label="Approved tattoo booking calendar specimen">' +
      '<div class="booking-main-column"><section class="section session-section"><div class="session-list"><button class="choice session-choice" type="button" aria-pressed="false"><span><span class="choice-title">Extended Day Session</span><span class="choice-meta">8–12 hours / Optional 8–12 hour session. Reserves a 12-hour appointment block with a $200 Extended Day fee.</span></span><span class="price-wrap"><span class="price">$350</span><span class="price-label">Deposit</span></span></button></div></section>' +
