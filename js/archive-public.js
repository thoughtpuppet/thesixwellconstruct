(function () {
  ...
  function editorialEvidence(block={}){const value=text(block.evidence_status,block.evidenceStatus,"documented");return `<span class="archive-editorial-evidence" data-evidence="${escapeHtml(value)}">${escapeHtml(value.replace("-"," "))}</span>`}
  function editorialGalleryMarkup(source={},behavior="static") {const items=list(source.items),detail=text(source.presentation_mode,source.presentationMode)==="detail-crop",focalX=Math.min(1,Math.max(0,Number(source.presentation_focal_x??source.presentationFocalX??0.5))),focalY=Math.min(1,Math.max(0,Number(source.presentation_focal_y??source.presentationFocalY??0.5))),ratio=Math.max(0.1,Number(source.presentation_aspect_ratio??source.presentationAspectRatio??1)),zoom=Math.min(10,Math.max(1,Number(source.presentation_zoom??source.presentationZoom??1))),detailAlt=text(source.presentation_alt_text,source.presentationAltText);return `<figure class="archive-editorial-montage${detail?" is-detail-crop":""}"${detail?` style="--editorial-detail-x:${focalX*100}%;--editorial-detail-y:${focalY*100}%;--editorial-detail-ratio:${ratio};--editorial-detail-zoom:${zoom}"`:""}>${items.map(item=>{const url=safeUrl(item.url),poster=safeUrl(first(item.poster_url,item.posterUrl)),alt=detail&&detailAlt?detailAlt:text(item.alt_text,item.altText,item.title);if(!url)return"";if(String(item.mime_type||item.mimeType||"").startsWith("video/")){return `<div class="archive-editorial-video">${poster?`<img src="${escapeHtml(poster)}" alt="${escapeHtml(alt)}" loading="lazy" data-loop-poster>`:""}<video muted loop playsinline preload="metadata" ${poster?`poster="${escapeHtml(poster)}"`:""} data-editorial-loop="${behavior==="muted-loop"?"1":"0"}" aria-label="${escapeHtml(alt)}"><source src="${escapeHtml(url)}" type="${escapeHtml(item.mime_type||item.mimeType||"video/mp4")}"></video>${item.transcript?`<details><summary>Transcript</summary><p>${escapeHtml(item.transcript)}</p></details>`:""}</div>`}return detail?`<div class="archive-editorial-detail-frame"><img src="${escapeHtml(url)}" alt="${escapeHtml(alt)}" loading="lazy"></div>`:`<img src="${escapeHtml(url)}" alt="${escapeHtml(alt)}" loading="lazy">`}).join("")}<figcaption>${escapeHtml(text(source.title))}${source.summary?` · ${escapeHtml(source.summary)}`:""}</figcaption></figure>`}
  function editorialObjectMarkup(block,collection=false){const item=block.source||{},image=safeUrl(item.image_url),meta=[item.period_label,item.item_size,item.colorway,item.technique].filter(Boolean).map(escapeHtml).join(" · "),sold=Boolean(item.is_sold||item.isSold),route=collection?safeUrl(item.route):safeUrl(item.route)||`/merch/${encodeURIComponent(item.slug||"")}/`;return `<article class="archive-editorial-object ${collection?"is-collection":"is-sale"}" ${!collection&&item.slug?`data-editorial-merch-slug="${escapeHtml(item.slug)}"`:""}>${image?`<img src="${escapeHtml(image)}" alt="${escapeHtml(item.alt_text||item.title||"")}" loading="lazy">`:""}<div><span class="archive-label">${collection?"Permanent collection":"From the Archive"}</span><h3>${escapeHtml(item.title||block.title||"Garment")}</h3>${meta?`<p>${meta}</p>`:""}${item.description?`<p>${escapeHtml(item.description)}</p>`:""}${item.provenance_summary?`<p>${escapeHtml(item.provenance_summary)}</p>`:""}<p class="archive-editorial-price" data-current-price>${collection?"Permanently non-saleable":sold?"Sold":"Current price in Merch"}</p>${item.historical_price_amount!=null?`<p>Documented historical price: ${escapeHtml(`${(Number(item.historical_price_amount)/100).toLocaleString("en-US",{style:"currency",currency:item.historical_price_currency||"USD"})}${item.historical_price_note?` · ${item.historical_price_note}`:""}`)}</p>`:""}${route?`<a class="archive-button" href="${escapeHtml(route)}">${collection?"Open promoted dossier":"View in Merch"}</a>`:""}</div></article>`}
  function editorialBlockMarkup(block,index){const type=text(block.block_type,block.blockType),source=block.source||{},title=text(block.title,source.title),body=text(block.body),date=text(block.date_label,block.dateLabel,source.reflected_on_label,source.reflectedOnLabel);let content="";if(type==="reflection")content=`<blockquote><p>${escapeHtml(text(block.excerpt,source.excerpt))}</p><footer>${source.route?`<a href="${escapeHtml(source.route)}">Read the complete Journal entry</a>`:""}<span>Written ${escapeHtml(dateLabel({date:source.source_created_at||source.sourceCreatedAt}))}</span></footer></blockquote>`;else if(type==="gallery-set")content=editorialGalleryMarkup(source,text(block.media_behavior,block.mediaBehavior));else if(type==="merch-item")content=editorialObjectMarkup(block,false);else if(type==="collection-item")content=editorialObjectMarkup(block,true);else if(type==="activity")content=timelineEntryMarkup(source,index);else content=`<div class="archive-editorial-open"><h3>${escapeHtml(title||"Open interval")}</h3>${body?paragraphMarkup(body):""}</div>`;return `<div class="archive-editorial-block is-${escapeHtml(type)}">${editorialEvidence(block)}${date?`<span class="archive-date">${escapeHtml(date)}</span>`:""}${content}${block.source_url?`<p class="archive-timeline-source"><a href="${escapeHtml(safeUrl(block.source_url))}">${escapeHtml(block.citation_label||"View source")}</a></p>`:""}</div>`}
  function editorialActMarkup(act,index){return `<section class="archive-editorial-act" id="timeline-${escapeHtml(slugify(act.anchor_slug||act.id||`act-${index+1}`))}"><header><span class="archive-kicker">${escapeHtml(act.eyebrow||act.chapter_role||"Act")}</span><span class="archive-editorial-index">${String(index+1).padStart(2,"0")}</span><h2>${escapeHtml(act.title)}</h2>${act.summary?`<p>${escapeHtml(act.summary)}</p>`:""}${act.body?`<div>${paragraphMarkup(act.body)}</div>`:""}</header><div class="archive-editorial-blocks">${list(act.blocks).map(editorialBlockMarkup).join("")}</div></section>`}
  function bindEditorialTimeline(){
    const reduced=matchMedia("(prefers-reduced-motion: reduce)").matches;
    document.querySelectorAll("[data-editorial-loop='1']").forEach(video=>{const poster=video.parentElement?.querySelector("[data-loop-poster]");if(reduced){video.hidden=true;if(poster)poster.hidden=false;return}if(poster)poster.hidden=true;video.play().catch(()=>{video.controls=true;if(poster)poster.hidden=false})});
    document.querySelectorAll("[data-editorial-merch-slug]").forEach(async card=>{try{const response=await fetch(`/api/shop/items/${encodeURIComponent(card.dataset.editorialMerchSlug)}`);if(!response.ok)return;const product=(await response.json()).product,price=card.querySelector("[data-current-price]");if(!price||!product)return;if(product.availabilityState==="sold_out")price.textContent="Sold";else if(product.price)price.textContent=new Intl.NumberFormat("en-US",{style:"currency",currency:product.price.currencyCode||"USD"}).format(Number(product.price.amount))}catch{}});
  }
  ...
  async function timeline() {
    ...
    try {
      ...
      const dossierRoute = safeUrl(first(timelineRecord.dossier_route, timelineRecord.dossierRoute, payload.dossier_route, payload.dossierRoute));
      if(text(timelineRecord.presentation_mode,timelineRecord.presentationMode)==="editorial"){
        const acts=list(payload.acts);
        app.innerHTML=`<header class="archive-timeline-header archive-editorial-header site-hero site-hero--supporting"><div><span class="archive-kicker">Living editorial history</span><h1 class="archive-timeline-title hero-title">${escapeHtml(title)}</h1></div><div class="archive-record-orientation">${intro?`<p class="archive-timeline-intro hero-descriptor">${escapeHtml(intro)}</p>`:""}<div class="archive-meta"><span>Published acts</span><span>Acts remain flexible as evidence emerges</span></div><div class="archive-actions">${dossierRoute?`<a class="archive-button" href="${escapeHtml(dossierRoute)}">Read identity dossier</a>`:""}${profileRoute?`<a class="archive-button" href="${escapeHtml(profileRoute)}">Read identity profile</a>`:""}<a class="archive-button" href="${escapeHtml(subjectExplorerUrl({...timelineRecord,slug}))}">Explore related records</a></div></div></header>${acts.length?`<div class="archive-editorial-timeline">${acts.map(editorialActMarkup).join("")}</div>`:`<section class="archive-empty"><span class="archive-kicker">Timeline forming</span><h2>No public acts yet.</h2><p>The address is ready while evidence and first-person passages are reviewed.</p></section>`}`;
        bindEditorialTimeline();return;
      }
