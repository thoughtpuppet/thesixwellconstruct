(function () {
  ...
  function explorer() {
    ...
    app.innerHTML = `
      <section class="archive-hero" aria-labelledby="archive-title">
        <div><span class="archive-kicker">Public living archive</span><h1 id="archive-title">${collectionsView ? "Collections." : "Archive."}</h1></div>
        <div class="archive-hero-copy"><p>${collectionsView ? "Move through records gathered around a shared body of work, period, place, or question." : "Search the work through its process, materials, people, places, and changing history."}</p>
  ...
  function materialMarkup(material, index) {
  ...
  async function dossier() {
    ...
    try {
      ...
      app.innerHTML = `
        <nav class="archive-breadcrumbs" aria-label="Breadcrumb"><a href="/archive/">Archive</a><span aria-hidden="true">/</span><span aria-current="page">${escapeHtml(title)}</span></nav>
        <article>
  ...
  async function timeline() {
    ...
    try {
      ...
      app.innerHTML = `<nav class="archive-breadcrumbs" aria-label="Breadcrumb"><a href="/archive/">Archive</a><span aria-hidden="true">/</span><span>Timelines</span><span aria-hidden="true">/</span><span aria-current="page">${escapeHtml(title)}</span></nav>
        <header class="archive-timeline-header"><div><span class="archive-kicker">Interactive history</span><h1 class="archive-timeline-title">${escapeHtml(title)}</h1></div><div class="archive-record-orientation">${intro ? `<p class="archive-timeline-intro">${escapeHtml(intro)}</p>` : ""}<div class="archive-meta"><span>${entries.length} ${entries.length === 1 ? "entry" : "entries"}</span>${timelineRecord.updated_at ? `<span>Updated ${escapeHtml(dateLabel({ date: timelineRecord.updated_at }))}</span>` : ""}</div><div class="archive-actions"><a class="archive-button" href="${escapeHtml(subjectExplorerUrl({ ...timelineRecord, slug }))}">Explore related records</a></div></div></header>
