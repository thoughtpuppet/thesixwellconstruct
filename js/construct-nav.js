...
(function() {
  ...
  function normalizedRoutePath(route) {
    try {
      var path = new URL(route, window.location.origin).pathname;
      return path.replace(/\/+$/, '') || '/';
    } catch (error) {
  ...
  function mountManagedFooterLinks(links) {
    ...
    function mount() {
      ...
      var linkContainer = footer.querySelector('nav[aria-label="Footer"], .page-links') || footer;
    ...
    var footerObserver = new MutationObserver(function() {
      if (mount()) footerObserver.disconnect();
    });
  ...
  (async function hydrateManagedConstructNav() {
    ...
    mountManagedFooterLinks(utilityLinks);
