...
async function getCandidate(db, id, includeRevisions = true) {
  ...
  if (!candidate || !includeRevisions) return candidate;
...
async function createCandidate(db, body, discoveredBy = "manual", provenance = []) {
  ...
  await appendRevision(db, id, candidateSnapshot(proposal), provenance, "Initial candidate", discoveredBy);
  return { candidate: await getCandidate(db, id), duplicate };
}

async function saveCandidate(db, id, body) {
  ...
  if (before !== after) await appendRevision(db, id, candidateSnapshot(proposal), [{ url: proposal.sourceUrl, savedAt: now }], "Studio edit");
  ...
  if (existing) {
    await db.prepare(
      `UPDATE calendar_entries SET sequence=?,status='published',source_url=?,ticket_url=?,title=?,organizer=?,
       ...
       subjects_json=?,formats_json=?,is_experimental=?,last_modified_at=?,last_verified_at=? WHERE id=?`
    ).bind(
      ...
      JSON.stringify(candidate.formats), candidate.experimental ? 1 : 0, now, candidate.lastVerifiedAt, entryId
    ).run();
  } else {
    await db.prepare(
      `INSERT INTO calendar_entries
       (id,candidate_id,uid,sequence,status,source_url,ticket_url,title,organizer,factual_description,date_kind,
        starts_at,ends_at,timezone,venue_name,venue_address,city,region,subjects_json,formats_json,is_experimental,
        published_at,last_modified_at,last_verified_at)
       VALUES (?,?,?,0,'published',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
    ).bind(
      entryId, id, uid, candidate.sourceUrl, candidate.ticketUrl, candidate.title, candidate.organizer,
      candidate.factualDescription, candidate.dateKind, candidate.startsAt, candidate.endsAt, candidate.timezone,
      candidate.venueName, candidate.venueAddress, candidate.city, candidate.region, JSON.stringify(candidate.subjects),
      JSON.stringify(candidate.formats), candidate.experimental ? 1 : 0, now, now, candidate.lastVerifiedAt
    ).run();
  }
  await db.prepare(
...
function curatedPublicView(row) {
  return {
    id: `curated:${row.id}`,
    origin: "curated",
    title: row.title,
...
async function loadCuratedEvents(db) {
  const result = await db.prepare("SELECT * FROM calendar_entries ORDER BY starts_at ASC,title ASC").all();
  return (result.results || []).map(curatedPublicView);
}
...
async function handleCandidates(request, env, parts) {
  ...
  if (!id) {
    ...
    if (method === "POST") {
      ...
      try { return json(await createCandidate(db, body, "manual", [{ url: body.sourceUrl || "", enteredAt: isoNow() }]), { status: 201 }); }
  ...
  if (!action) {
    ...
    if (method === "PATCH") {
      const body = await readBody(request);
      if (!body) return errorResponse("Invalid JSON body.");
      const candidate = await saveCandidate(db, id, body);
      return candidate ? json({ candidate }) : errorResponse("Candidate not found.", 404);
    }
    return errorResponse("Method not allowed.", 405);
  }
  ...
  if (action === "duplicate") {
    const candidate = await saveCandidate(db, id, { status: "duplicate", duplicateOf: body.duplicateOf || "" });
    return candidate ? json({ candidate }) : errorResponse("Candidate not found.", 404);
  }
...
function structuredEventProposal(item, source) {
  ...
  return {
    ...
    sourceUrl: asString(item.url) || source.url, ticketUrl: asString(offers.url), title: asString(item.name),
...
async function upsertScoutProposal(db, rawProposal, discoveredBy, provenance, profile) {
  ...
  await saveCandidate(db, current.id, { ...proposal, status: current.status === "published" ? "published" : "candidate" });
  return { candidate: await getCandidate(db, current.id, false), existing: true, changed: true };
}

async function monitorSources(db, profile) {
  ...
  for (const source of sources) {
    ...
    try {
      ...
      for (const proposal of proposals) {
        const stored = await upsertScoutProposal(db, proposal, "source_monitor", [{ url: proposal.sourceUrl, retrievedAt: now }], profile);
...
async function runOpenAiDiscovery(env, db, profile) {
  ...
  for (const event of (Array.isArray(parsed.events) ? parsed.events : []).slice(0, profile.perRunLimit)) {
    try {
      const stored = await upsertScoutProposal(db, event, "openai_web_search", citations, profile);
...
export async function runCalendarScout(env, { runKind = "scheduled", includeWeb = true } = {}) {
  ...
  let fatalError = "";
  try {
    sourceResult = await monitorSources(db, profile);
    if (includeWeb) {
