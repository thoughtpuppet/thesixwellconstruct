(function(){
  ...
  const managed={tattoo:new Set(["flash"]),art:new Set(["works"]),merch:new Set(["products"]),legend:new Set(["symbols","categories","composition-rules","themes","examples","usage","drafts"]),events:new Set(["event-archive"]),archive:new Set(["dossiers","blackboards","origin-threads","records","collections","timeline","people","places","media-artifacts","drafts","settings"]),site:new Set(["pathways","nodes","navigation","search","visibility","settings"]),shared:new Set(["media","relationships","taxonomy","revisions","search-index"])};
  ...
  const archiveWorkspace={dossiers:[],media:[],originThreads:[],catalogueMedia:[],objectTypes:[],documentationFields:[],documentation:[],entities:[],selectedEntityId:"",detail:null,materials:[],sourceMaterials:[],activities:[],versions:[],states:[],filter:"",previewUrls:[]};
  ...
  function catalogueForm(dossier){
    ...
    return `<form class="cm-form" data-dossier-catalogue><div class="cm-catalogue-preview"><span class="cm-section-index">Permanent catalogue identity</span><strong data-catalogue-preview>${esc(catalogueLabel(dossier))}</strong><small>${esc(firstValue(dossier,"catalogue_medium_label","medium_label")||"Cultural object")} · ${esc(firstValue(dossier,"cultural_object_type","object_type_label")||"Needs classification")}</small></div><div class="cm-form-grid"><label>Medium / catalogue family<select name="medium_id">${archiveWorkspace.catalogueMedia.map(item=>option(firstValue(item,"id"),firstValue(item,"label"),medium)).join("")}</select></label><label>Cultural object type<select name="object_type_id">${catalogueObjectTypeOptions(medium,objectType)}</select></label><label>Permanent sequence number<input name="catalogue_number" type="number" value="${esc(number)}" readonly aria-readonly="true"></label><label class="wide">Current public condition<select name="current_state_id"><option value="">No current public condition selected</option>${stateOptions}</select></label></div><p class="cm-meta">The assigned prefix and base number are permanent. Classifications may change only when the catalogue prefix remains unchanged. The current condition is selected from real published states and keeps legacy catalogue fields synchronized.</p><p class="cm-current-condition"><strong>Current public condition:</strong> <span data-current-condition-label>${esc(currentStateId?catalogueLabel(dossier):"Not selected")}</span></p><p class="cm-meta" data-state-guidance>${esc(firstValue(dossier,"state_guidance")||"Choose the object type to see medium-specific state guidance.")}</p><div class="cm-actions"><button class="button" type="submit">Save catalogue identity</button><span class="cm-upload-status" data-dossier-catalogue-status aria-live="polite"></span></div></form>`;
  }
  ...
  async function loadArchiveDossier(entityId){
    ...
    try{
      const [detailPayload,materialPayload,sourceMaterialPayload,activityPayload,mediaPayload,threadPayload,cataloguePayload,entityPayload]=await Promise.all([api(archiveEndpoints.dossier(entityId)),api(queryEndpoint(archiveEndpoints.materials,{entity_id:entityId})),api(queryEndpoint(archiveEndpoints.sourceMaterials,{entity_id:entityId})),api(queryEndpoint(archiveEndpoints.activities,{entity_id:entityId})),api(archiveEndpoints.media),api(archiveEndpoints.originThreads),api(archiveEndpoints.catalogue),api("/api/admin/entities")]),listRecord=archiveWorkspace.dossiers.find(item=>archiveEntityId(item)===entityId)||{},dossier={...listRecord,...recordFrom(detailPayload,"dossier")};
      archiveWorkspace.detail=dossier;archiveWorkspace.materials=recordsFrom(materialPayload,"materials");archiveWorkspace.sourceMaterials=recordsFrom(sourceMaterialPayload,"source_materials");archiveWorkspace.activities=recordsFrom(activityPayload,"activities");archiveWorkspace.media=recordsFrom(mediaPayload,"media");archiveWorkspace.originThreads=recordsFrom(threadPayload,"origin_threads");archiveWorkspace.catalogueMedia=recordsFrom(cataloguePayload,"media");archiveWorkspace.objectTypes=recordsFrom(cataloguePayload,"object_types");archiveWorkspace.documentationFields=recordsFrom(cataloguePayload,"documentation_fields");archiveWorkspace.documentation=parseList(detailPayload.documentation||firstValue(dossier,"documentation"));archiveWorkspace.entities=recordsFrom(entityPayload,"entities");archiveWorkspace.versions=recordsFrom(detailPayload,"versions");archiveWorkspace.states=recordsFrom(detailPayload,"states");
  ...
  function bindDossierWorkspace(shell){
    ...
    shell.addEventListener("submit",async event=>{
      ...
      if(catalogue){const output=catalogue.querySelector("[data-dossier-catalogue-status]"),data=new FormData(catalogue),payload={medium_id:String(data.get("medium_id")||""),object_type_id:String(data.get("object_type_id")||""),catalogue_number:Number(data.get("catalogue_number"))||0,current_state_id:String(data.get("current_state_id")||"")||null};try{output.textContent="Saving catalogue identity…";await archiveJson(archiveEndpoints.catalogueItem(entityId),"PATCH",payload);status("Catalogue identity saved");await loadArchiveDossier(entityId)}catch(error){output.textContent=error.message;status(error.message)}return}
