(function(){
  ...
  const managed={tattoo:new Set(["flash","designs"]),art:new Set(["works"]),merch:new Set(["products"]),legend:new Set(["symbols","categories","composition-rules","themes","examples","usage","drafts"]),about:new Set(["appearances"]),events:new Set(["event-archive"]),archive:new Set(["dossiers","notes","failed-experiments","colors-materials","blackboards","origin-threads","records","collections","timeline","people","organizations","places","media-artifacts","drafts","settings"]),site:new Set(["pathways","nodes","navigation","search","visibility","settings"]),shared:new Set(["media","relationships","taxonomy","revisions","search-index"])};
  ...
  window.ConstructManager={isManagedView:(tab,view)=>managed[tab]?.has(view)||false,render:(tab,view)=>tab==="merch"&&view==="products"?import("/studio/merch-manager.js?v=20260813-merch-image-reference").then(module=>module.mount(root(),api,status)):tab==="about"&&view==="appearances"?renderResource("appearances"):tab==="archive"&&view==="organizations"?renderResource("organizations"):render(tab,view)};
})();
