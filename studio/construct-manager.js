(function(){
  ...
  const managed={tattoo:new Set(["flash","designs"]),art:new Set(["works"]),merch:new Set(["products"]),legend:new Set(["symbols","categories","composition-rules","themes","examples","usage","drafts"]),about:new Set(["current-works","creative-identities","appearances"]),events:new Set(["event-archive"]),archive:new Set(["dossiers","notes","failed-experiments","colors-materials","blackboards","origin-threads","records","legacy-records","collections","timeline","people","organizations","places","media-artifacts","drafts","settings"]),site:new Set(["pathways","nodes","navigation","search","visibility","settings"]),shared:new Set(["media","relationships","taxonomy","revisions","search-index"])};
  ...
  function sourceMaterialForm(sourceMaterial={},isNew=false){
    const id=firstValue(sourceMaterial,"id"),visibility=firstValue(sourceMaterial,"visibility")||"internal",publicationState=firstValue(sourceMaterial,"publication_state","publicationState")||"draft",sourceKind=firstValue(sourceMaterial,"source_kind","sourceKind")||"client-correspondence";
  ...
  function dossierWorkspaceMarkup(dossier,materials,activities,media){
    ...
    const identitySummary=contextualEvent
      ?"A contextual Event record with its own EVT authority identity, evidence, relationships, and publication controls."
      :identityOrganization
        ...
        :"One cultural object with its own catalogue identity, evolution, evidence, context, relationships, and publication controls.";
    ...
    const identitySections=contextualEvent
      ...
      :identityOrganization
        ?`<section class="cm-workspace-section" id="cm-dossier-identity-context"><div class="cm-workspace-section-head"><div><span class="cm-section-index">01 · Creative identity</span><h3>Organization evidence record</h3><p>This dossier belongs to the canonical organization. Its About profile controls identity publication; this Archive record holds evidence and never receives a cultural-object catalogue number.</p></div></div></section>`
      ...
      <section class="cm-workspace-section" id="cm-dossier-documentation"><div class="cm-workspace-section-head"><div><span class="cm-section-index">03 · Documentation</span><h3>Adaptive catalogue documentation</h3><p>Add only applicable facts. Canonical title, date, medium, artist, organizations, production roles, state changes, and Digital-asset privacy, presentation, and rights remain authoritative in their existing systems.</p></div></div>${documentationMarkup()}</section>`;
  ...
  function serializeMaterialForm(form,entityId){
    ...
    if(payload.state==="published"&&payload.visibility!=="public")throw new Error("Public material must use Public visibility.");
  ...
  async function render(tab,view){if(!(tab==="archive"&&(view==="dossiers"||view==="drafts")))cancelArchiveDossierOpen();if(tab==="tattoo"&&configs[view])return renderResource(view);if(tab==="art"&&view==="works")return renderResource("works");if(tab==="legend"){if(view==="symbols")return renderResource("symbols");if(view==="categories")return renderResource("categories");if(view==="composition-rules")return renderCompositionRules();if(view==="themes")return renderLegendFacet("Themes");if(view==="examples")return renderLegendFacet("Examples");if(view==="usage")return renderLegendUsage();return renderResource("symbols",record=>record.state==="draft")}if(tab==="events"&&view==="event-archive")return renderResource("records",record=>record.record_type==="event");if(tab==="archive"){if(view==="dossiers")return renderArchiveDossiers();if(view==="notes")return import("/studio/archive-notes-manager.js?v=20260828-journal-batch").then(module=>module.mountArchiveNotes(root(),api,status));if(view==="failed-experiments")return window.ArchiveFailedExperimentsStudio?.mount(root(),api,status);if(view==="colors-materials")return window.ArchiveColorMaterialsStudio?.mount(root(),api,status);if(view==="blackboards")return import("/studio/archive-blackboards-manager.js?v=20260828-fragment-batch").then(module=>module.mountArchiveBlackboards(root(),api,status));if(view==="origin-threads")return renderOriginThreads();if(view==="legacy-records")return renderResource("records");if(["collections","people","places"].includes(view))return renderResource(view);if(view==="media-artifacts")return renderMedia();if(view==="timeline")return renderArchiveTimelines();if(view==="drafts")return renderArchiveDossiers("draft");return renderSimple("Archive Settings","/api/admin/archive-dossiers","Canonical item shells, material privacy, presentation, relationship visibility, and publication state.")}if(tab==="site"){if(view==="nodes"||view==="pathways")return renderResource(view);if(view==="navigation")return renderPreviews("Navigation Preview and Rollback");if(view==="search")return renderPreviews("Search Preview");if(view==="visibility")return import("/studio/page-visibility-manager.js?v=20260821-studio-visibility").then(module=>module.mountPageVisibility(root(),api,status));return renderPreviews("Site Settings and Preview Hub")}if(tab==="shared"){if(view==="media")return renderMedia();if(view==="relationships")return renderRelationships();if(view==="revisions")return renderSimple("Revision History","/api/admin/revisions","Immutable snapshots for audit, comparison, and restore-as-new-draft.");if(view==="search-index")return renderSimple("Search Index Status","/api/admin/search/status","Published records and unresolved indexing failures.");return renderSimple("Tags and Themes","/api/admin/taxonomy","Controlled terms used across entities.")}}
  window.ConstructManager={isManagedView:(tab,view)=>managed[tab]?.has(view)||false,cancelArchiveDossierOpen,openArchiveDossier,render:(tab,view)=>tab==="merch"&&view==="products"?import("/studio/merch-manager.js?v=20260828-editable-archive-records").then(module=>module.mount(root(),api,status)):tab==="about"&&view==="current-works"?import("/studio/current-works-manager.js?v=20260826-current-works").then(module=>module.mountCurrentWorks(root(),api,status)):tab==="about"&&view==="creative-identities"?import("/studio/creative-identities-manager.js?v=20260828-creative-identities").then(module=>module.mountCreativeIdentities(root(),api,status)):tab==="about"&&view==="appearances"?renderResource("appearances"):tab==="archive"&&view==="records"?import("/studio/creative-identities-manager.js?v=20260828-creative-identities").then(module=>module.mountCulturalObjectCreator(root(),api,status)):tab==="archive"&&view==="organizations"?renderResource("organizations"):render(tab,view)};
})();
