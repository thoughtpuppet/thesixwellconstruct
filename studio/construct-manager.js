(function(){
  ...
  function editor(config,record={},categories=[],styleOptions=[]){
    ...
    const fields=config.fields.map(name=>config.flashEditor?flashField(name,record[name]??(name==="state"?"draft":""),!record.id,record):config===configs.works?artworkField(name,record[name]??(name==="state"?"draft":""),record):field(name,record[name]??(name==="state"?"draft":""))).join("");
  ...
  const archiveWorkspace={dossiers:[],media:[],originThreads:[],catalogueMedia:[],objectTypes:[],entities:[],selectedEntityId:"",detail:null,materials:[],activities:[],versions:[],states:[],filter:"",previewUrls:[]};
  ...
  function archiveDossierCard(record){
    ...
    const mediumLabel=contextualEvent?"Events":firstValue(record,"catalogue_medium_label","catalogueMediumLabel")||archiveType(record),recordClass=contextualEvent?"Contextual Archive record":firstValue(record,"cultural_object_type","culturalObjectType")||"Cultural object";
  ...
  function catalogueLabel(record){
    const base=firstValue(record,"catalogue_id","catalogueId"),version=Number(firstValue(record,"current_version","currentVersion"))||1,state=firstValue(record,"current_state","currentState")||"I",variant=firstValue(record,"catalogue_variant","variant_label","variantLabel");
    return base?`${base}.${version}/${state}${variant?`, ${variant}`:""}`:"Catalogue identity pending";
  }
  ...
  function catalogueForm(dossier){
    const medium=firstValue(dossier,"catalogue_medium","medium_id","mediumId")||archiveWorkspace.catalogueMedia[0]?.id||"other",objectType=firstValue(dossier,"cultural_object_type_id","object_type_id","objectTypeId"),number=Number(firstValue(dossier,"catalogue_number","catalogueNumber"))||1,version=Number(firstValue(dossier,"current_version","currentVersion"))||1,stateValue=firstValue(dossier,"current_state","currentState")||"I";
    return `<form class="cm-form" data-dossier-catalogue><div class="cm-catalogue-preview"><span class="cm-section-index">Catalogue identity</span><strong data-catalogue-preview>${esc(catalogueLabel(dossier))}</strong><small>${esc(firstValue(dossier,"catalogue_medium_label","medium_label")||"Cultural object")} · ${esc(firstValue(dossier,"cultural_object_type","object_type_label")||"Needs classification")}</small></div><div class="cm-form-grid"><label>Medium / catalogue family<select name="medium_id">${archiveWorkspace.catalogueMedia.map(item=>option(firstValue(item,"id"),firstValue(item,"label"),medium)).join("")}</select></label><label>Cultural object type<select name="object_type_id">${catalogueObjectTypeOptions(medium,objectType)}</select></label><label>Sequence number<input name="catalogue_number" type="number" min="1" step="1" value="${esc(number)}" required></label><label>Current version<input name="current_version" type="number" min="1" step="1" value="${esc(version)}" required></label><label>Current state<input name="current_state" value="${esc(stateValue)}" pattern="[IVXLCDMivxlcdm]+" placeholder="II" required></label><label>Variant<input name="variant_label" value="${esc(firstValue(dossier,"catalogue_variant","variant_label","variantLabel"))}" placeholder="Optional: variant 1"></label></div><p class="cm-meta" data-state-guidance>${esc(firstValue(dossier,"state_guidance")||"Choose the object type to see medium-specific state guidance.")}</p><div class="cm-actions"><button class="button" type="submit">Save catalogue identity</button><span class="cm-upload-status" data-dossier-catalogue-status aria-live="polite"></span></div></form>`;
  }
  ...
  async function loadArchiveDossier(entityId){
    ...
    try{
      const [detailPayload,materialPayload,activityPayload,mediaPayload,threadPayload,cataloguePayload,entityPayload]=await Promise.all([api(archiveEndpoints.dossier(entityId)),api(queryEndpoint(archiveEndpoints.materials,{entity_id:entityId})),api(queryEndpoint(archiveEndpoints.activities,{entity_id:entityId})),api(archiveEndpoints.media),api(archiveEndpoints.originThreads),api(archiveEndpoints.catalogue),api("/api/admin/entities")]),listRecord=archiveWorkspace.dossiers.find(item=>archiveEntityId(item)===entityId)||{},dossier={...listRecord,...recordFrom(detailPayload,"dossier")};
      archiveWorkspace.detail=dossier;archiveWorkspace.materials=recordsFrom(materialPayload,"materials");archiveWorkspace.activities=recordsFrom(activityPayload,"activities");archiveWorkspace.media=recordsFrom(mediaPayload,"media");archiveWorkspace.originThreads=recordsFrom(threadPayload,"origin_threads");archiveWorkspace.catalogueMedia=recordsFrom(cataloguePayload,"media");archiveWorkspace.objectTypes=recordsFrom(cataloguePayload,"object_types");archiveWorkspace.entities=recordsFrom(entityPayload,"entities");archiveWorkspace.versions=recordsFrom(detailPayload,"versions");archiveWorkspace.states=recordsFrom(detailPayload,"states");
  ...
  function bindDossierWorkspace(shell){
    ...
    shell.addEventListener("change",event=>{
      ...
      const catalogueType=event.target.closest('[data-dossier-catalogue] select[name="object_type_id"]');if(catalogueType){const form=catalogueType.closest("form"),type=archiveWorkspace.objectTypes.find(item=>String(firstValue(item,"id"))===String(catalogueType.value));if(type){form.querySelector("[data-state-guidance]").textContent=firstValue(type,"state_guidance","stateGuidance")||"";const number=String(Number(form.elements.catalogue_number.value)||1).padStart(3,"0"),version=Number(form.elements.current_version.value)||1,stateValue=String(form.elements.current_state.value||"I").toUpperCase(),variant=String(form.elements.variant_label.value||"").trim();form.querySelector("[data-catalogue-preview]").textContent=`${firstValue(type,"catalogue_prefix","cataloguePrefix")}-${number}.${version}/${stateValue}${variant?`, ${variant}`:""}`};return}
    ...
    shell.addEventListener("input",event=>{const eventForm=event.target.closest("[data-event-identifier]");if(eventForm){eventForm.querySelector("[data-event-identifier-preview]").textContent=`EVT-${String(Number(eventForm.elements.event_number.value)||1).padStart(3,"0")}`;return}const form=event.target.closest("[data-dossier-catalogue]");if(!form)return;const type=archiveWorkspace.objectTypes.find(item=>String(firstValue(item,"id"))===String(form.elements.object_type_id.value));if(!type)return;const number=String(Number(form.elements.catalogue_number.value)||1).padStart(3,"0"),version=Number(form.elements.current_version.value)||1,stateValue=String(form.elements.current_state.value||"I").toUpperCase(),variant=String(form.elements.variant_label.value||"").trim();form.querySelector("[data-catalogue-preview]").textContent=`${firstValue(type,"catalogue_prefix","cataloguePrefix")}-${number}.${version}/${stateValue}${variant?`, ${variant}`:""}`});
    shell.addEventListener("submit",async event=>{
      event.preventDefault();const eventIdentifier=event.target.closest("[data-event-identifier]"),catalogue=event.target.closest("[data-dossier-catalogue]"),context=event.target.closest("[data-dossier-context]"),versionFormElement=event.target.closest("[data-version-form]"),stateFormElement=event.target.closest("[data-state-form]"),story=event.target.closest("[data-dossier-story]"),origins=event.target.closest("[data-dossier-origins]"),publish=event.target.closest("[data-dossier-publish]"),material=event.target.closest("[data-material-form]"),activity=event.target.closest("[data-activity-form]");
      ...
      if(catalogue){const output=catalogue.querySelector("[data-dossier-catalogue-status]"),data=new FormData(catalogue),payload={medium_id:String(data.get("medium_id")||""),object_type_id:String(data.get("object_type_id")||""),catalogue_number:Number(data.get("catalogue_number"))||0,current_version:Number(data.get("current_version"))||1,current_state:String(data.get("current_state")||"I").toUpperCase(),variant_label:String(data.get("variant_label")||"").trim()};try{output.textContent="Saving catalogue identity…";await archiveJson(archiveEndpoints.catalogueItem(entityId),"PATCH",payload);status("Catalogue identity saved");await loadArchiveDossier(entityId)}catch(error){output.textContent=error.message;status(error.message)}return}
      ...
      if(versionFormElement){const output=versionFormElement.querySelector("[data-version-status]"),data=new FormData(versionFormElement),id=versionFormElement.dataset.id,payload={entity_id:entityId,version_number:Number(data.get("version_number"))||1,title:String(data.get("title")||"").trim(),description:String(data.get("description")||"").trim(),occurred_at:String(data.get("occurred_at")||"")||null,date_precision:String(data.get("date_precision")||"undated"),date_label:String(data.get("date_label")||"").trim(),sort_order:Number(data.get("sort_order"))||0};try{output.textContent="Saving version…";await archiveJson(id?archiveEndpoints.version(id):archiveEndpoints.versions,id?"PATCH":"POST",payload);status(id?"Version saved":"Version added");await loadArchiveDossier(entityId)}catch(error){output.textContent=error.message;status(error.message)}return}
      if(stateFormElement){const output=stateFormElement.querySelector("[data-state-status]"),data=new FormData(stateFormElement),id=stateFormElement.dataset.id,payload={version_id:stateFormElement.dataset.versionId,state_roman:String(data.get("state_roman")||"I").toUpperCase(),state_order:Number(data.get("state_order"))||1,title:String(data.get("title")||"").trim(),description:String(data.get("description")||"").trim(),variant_label:String(data.get("variant_label")||"").trim(),occurred_at:String(data.get("occurred_at")||"")||null,date_precision:String(data.get("date_precision")||"undated"),date_label:String(data.get("date_label")||"").trim(),sort_order:Number(data.get("sort_order"))||0};try{output.textContent="Saving state…";await archiveJson(id?archiveEndpoints.state(id):archiveEndpoints.states,id?"PATCH":"POST",payload);status(id?"State saved":"State added");await loadArchiveDossier(entityId)}catch(error){output.textContent=error.message;status(error.message)}return}
