(function(){
  ...
  const managed={tattoo:new Set(["flash","designs"]),art:new Set(["works"]),merch:new Set(["products"]),legend:new Set(["symbols","categories","composition-rules","themes","examples","usage","drafts"]),about:new Set(["current-works","appearances"]),events:new Set(["event-archive"]),archive:new Set(["dossiers","notes","failed-experiments","colors-materials","blackboards","origin-threads","records","collections","timeline","people","organizations","places","media-artifacts","drafts","settings"]),site:new Set(["pathways","nodes","navigation","search","visibility","settings"]),shared:new Set(["media","relationships","taxonomy","revisions","search-index"])};
  ...
  async function saveAppearanceEditor(form){
    ...
    try{
      ...
      const saved=await api(`/api/admin/appearances${originalId?`/${encodeURIComponent(originalId)}`:""}`,{method:originalId?"PATCH":"POST",headers:{"content-type":"application/json"},body:JSON.stringify(record)}),entityId=originalId||saved.record?.id;if(!entityId)throw new Error("Appearance saved without an entity ID.");
      let dossier;try{dossier=await api(`/api/admin/archive-dossiers/${encodeURIComponent(entityId)}`)}catch{dossier=await api("/api/admin/archive-dossiers",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({entity_id:entityId,archive_slug:record.slug||entityId,state:"draft",public_visible:false})})}
