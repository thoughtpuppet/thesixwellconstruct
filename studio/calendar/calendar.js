(function () {
  ...
  var state = { candidates:[], sources:[], socialSources:[], connectors:[], knownOrganizations:[], profile:null, suggestions:[], runs:[], filter:"review", selectedId:"", draftNew:false, broadDiscoveryEnabled:false, activeCandidate:null, flyerPreviewUrl:"" };
  ...
  async function connect() {
    ...
    try {
      var payload = await api("/api/admin/calendar"); localStorage.setItem(TOKEN_KEY, token); state.candidates=payload.candidates||[]; state.sources=payload.sources||[]; state.socialSources=payload.socialSources||[]; state.connectors=payload.connectors||[]; state.knownOrganizations=payload.knownOrganizations||[]; state.profile=payload.profile; state.broadDiscoveryEnabled=payload.broadDiscoveryEnabled;
      tokenInput.value=""; authPanel.hidden=true; app.hidden=false; renderStatusFilters(); renderCandidateList(); renderSources(); renderSocialSources(); renderConnectors(); renderProfile(); renderKnownOrganizations(); await Promise.all([loadSuggestions(),loadRuns()]);
    } catch (error) {
