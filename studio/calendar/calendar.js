(function () {
  ...
  function proposalMarkup(proposal){
    ...
    var changes=(proposal.changes||[]).map(function(change){var done=applied.includes(change.id);return '<label class="research-change'+(done?' is-applied':'')+'"><input type="checkbox" data-research-change="'+escapeHtml(change.id)+'"'+(!open||done?' disabled':'')+'><span><strong>'+escapeHtml(change.label||change.path)+'</strong><small>'+escapeHtml(changeValue(change.before))+' → '+escapeHtml(changeValue(change.value))+'</small><em>'+escapeHtml(change.rationale||"")+'</em><span class="research-confidence">Confidence '+Math.round((Number(change.confidence)||0)*100)+'%</span>'+researchCitations(change.citations)+'</span></label>';}).join("")||'<p class="section-guidance">The Scout answered without proposing record changes.</p>';
    return '<article class="research-proposal is-'+escapeHtml(proposal.state)+'"><div class="research-proposal-head"><strong>Proposed changes</strong><span>'+escapeHtml(proposal.state.replace(/_/g," "))+'</span></div>'+findings+'<div class="research-change-list">'+changes+'</div>'+(open?'<div class="research-proposal-actions"><button type="button" data-apply-research="'+escapeHtml(proposal.id)+'">Apply selected changes</button><button type="button" data-dismiss-research="'+escapeHtml(proposal.id)+'">Dismiss proposal</button></div>':'')+'</article>';
  }
  ...
  editorRoot.addEventListener("change",function(event){if(event.target.id==="candidateMediaFiles"&&event.target.files&&event.target.files.length)uploadMedia(Array.from(event.target.files)).catch(function(error){toast(error.message);});});
