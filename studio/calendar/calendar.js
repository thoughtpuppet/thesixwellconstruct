(function () {
  ...
  function renderEditor(candidate) {
    ...
    editorRoot.innerHTML = '<div class="editor-head"><div><p class="eyebrow">' + (isNew ? 'Manual intake' : recordLabel(candidate)) + '</p><h2>' + escapeHtml(candidate.title || "New candidate") + '</h2></div><span class="status-badge">' + escapeHtml(lifecycleLabel(candidate)) + '</span></div>' + pendingScheduleNotice(candidate) +
      ...
      field("candidateStartsAt",candidate.dateKind==="date_range"?"First date":candidate.dateKind==="all_day"?"Event date":"Starts",calendarControlValue(candidate.dateKind,candidate.startsAt),{type:candidate.dateKind==="timed"?"datetime-local":"date"}) + field("candidateEndsAt",candidate.dateKind==="date_range"?"Closing / final date (optional for exhibitions)":"Ends (optional)",calendarControlValue(candidate.dateKind,candidate.endsAt),{type:candidate.dateKind==="timed"?"datetime-local":"date"}) + field("candidateConfirmedThrough","Confirmed through (when closing date is unknown)",candidate.confirmedThrough,{type:"date"}) + field("candidateTimezone","Time zone",candidate.timezone) + '<div class="candidate-schedule-guidance is-wide" id="candidateScheduleGuidance"></div>' + field("candidateVenueName","Venue",candidate.venueName) +
