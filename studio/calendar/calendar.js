(function () {
  ...
  function occurrenceSection(candidate) {
    return '<div class="editor-section"><div class="section-title-row"><div><h3>Related schedule</h3><p class="section-guidance">Keep the exhibition or primary event above. Add its opening, closing reception, talks, screenings, workshops, and other dated programs here. Date-TBD items remain private until scheduled. Removing a program that was already published records it as cancelled in subscribed calendars.</p></div><button type="button" data-add-occurrence>Add occurrence</button></div><div class="occurrence-list" id="candidateOccurrences">'+((candidate.occurrences||[]).map(occurrenceRow).join("")||'<p class="occurrences-empty">No related schedule items.</p>')+'</div></div>';
  }
  ...
  function renderSources() {
    document.getElementById("sourceList").innerHTML = state.sources.length ? state.sources.map(function (source) {
      ...
      return '<details class="source-card source-disclosure" name="calendar-source-details" data-source-key="' + escapeHtml(sourceKey) + '" data-source-id="' + escapeHtml(source.id) + '"' + (state.openSourceKey===sourceKey?' open':'') + '>' +
        ...
        '<label class="field"><span>Adapter</span><select id="sourceAdapter-' + source.id + '">' + [["automatic","Automatic"],["beltline","Atlanta BeltLine"],["squarespace","Squarespace Events"],["high_art_making","High Art Making"],["eyedrum","Eyedrum"],["rampant","Rampant Gallery"],["eventbrite","Eventbrite discovery"],["posh","Posh discovery"],["bigtickets","BigTickets widget"],["partiful","Partiful event"],["wix","Wix"],["localist","Localist"],["out_of_hand","Out of Hand"],["json","JSON"],["icalendar","iCalendar"],["rss","RSS"]].map(function(option){return '<option value="'+option[0]+'"'+((source.adapterKey||"automatic")===option[0]?' selected':'')+'>'+option[1]+'</option>';}).join('') + '</select></label>' +
