(function () {
  ...
  function candidatePayload() {
    ...
    return {
      ...
      startsAt:calendarPayloadValue(value("candidateDateKind"),value("candidateStartsAt"),value("candidateTimezone")||"America/New_York"), endsAt:calendarPayloadValue(value("candidateDateKind"),value("candidateEndsAt"),value("candidateTimezone")||"America/New_York"), confirmedThrough:value("candidateConfirmedThrough"), visitingHours:collectVisitingHours(editorRoot), visitingHoursNote:value("candidateVisitingHoursNote"), visitingHoursSourceUrl:value("candidateVisitingHoursSourceUrl"), visitingHoursVerifiedAt:value("candidateVisitingHoursVerifiedAt"), timezone:value("candidateTimezone") || "America/New_York", venueName:value("candidateVenueName"),
  ...
  function publicationBlockers(candidate) {
    ...
    var seriesUsesOccurrenceVenues=candidate.eventStructure==="series"&&scheduledPublicOccurrences.length>0&&!(candidate.formats||[]).includes("exhibition")&&scheduledPublicOccurrences.every(function(occurrence){var occurrenceDelayed=occurrence.locationDisclosure==="after_registration"||(!occurrence.locationDisclosure&&candidate.locationDisclosure==="after_registration");var venueName=occurrence.venueName||candidate.venueName;var venueAddress=occurrence.venueAddress||candidate.venueAddress;var ticketUrl=occurrence.ticketUrl||candidate.ticketUrl;return Boolean(venueName&&(String(venueName).toLowerCase().includes("online")||String(venueAddress||"").toLowerCase()==="online"||venueAddress))||(occurrenceDelayed&&Boolean(ticketUrl)&&!isSocialUrl(ticketUrl));});
    ...
    if(!candidate.venueAddress&&!delayedLocation&&!seriesUsesOccurrenceVenues)blockers.push("Confirm the venue address.");
