  ...
  function renderSources() {
    document.getElementById("sourceList").innerHTML = state.sources.length ? state.sources.map(function (source) {
      ...
      return '<details class="source-card source-disclosure" name="calendar-source-details" data-source-key="' + escapeHtml(sourceKey) + '" data-source-id="' + escapeHtml(source.id) + '"' + (state.openSourceKey===sourceKey?' open':'') + '>' +
        ...
        '<label class="field"><span>Adapter</span><select id="sourceAdapter-' + source.id + '">' + [["automatic","Automatic"],["beltline","Atlanta BeltLine"],["squarespace","Squarespace Events"],["high_art_making","High Art Making"],["eyedrum","Eyedrum"],["rampant","Rampant Gallery"],["seven_stages","7 Stages / VBO performance runs"],["eventbrite","Eventbrite discovery"],["posh","Posh discovery"],["bigtickets","BigTickets widget"],["partiful","Partiful event"],["wix","Wix"],["localist","Localist"],["out_of_hand","Out of Hand"],["json","JSON"],["icalendar","iCalendar"],["rss","RSS"]].map(function(option){return '<option value="'+option[0]+'"'+((source.adapterKey||"automatic")===option[0]?' selected':'')+'>'+option[1]+'</option>';}).join('') + '</select></label>' +
