(function(){
  ...
  async function matchingSession(file,token){
    const records=saved(),key=fingerprint(file),sessionId=records[key];
    if(!sessionId)return null;
    try{
      const payload=await request(`/api/admin/media/uploads/${encodeURIComponent(sessionId)}`,token);
      const upload=payload.upload;
      if(upload?.state==="pending"&&upload.filename===file.name&&upload.mimeType===file.type&&upload.byteSize===file.size)return upload;
    }catch{}
  ...
  async function upload(file,options={}){
    ...
    if(!allowed.has(file.type))throw new Error("Use an MP4 or WebM video. MP4 with H.264/AAC is recommended.");
    if(file.size<=0||file.size>maxBytes)throw new Error("Videos must be 2 GiB or smaller.");
    const key=fingerprint(file),records=saved();
    let session=await matchingSession(file,token);
    if(!session){
      const created=await request("/api/admin/media/uploads",token,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
        filename:file.name,mimeType:file.type,byteSize:file.size,
    ...
    options.onStatus?.("Finalizing video…");
  ...
  window.StudioResumableMedia={upload,cancel,maxBytes,allowedTypes:[...allowed]};
})();
